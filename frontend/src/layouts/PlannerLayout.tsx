...
const PLANNER_NOTIF_ICON: Record<string, { icon: string; color: string }> = {
  vessel_arrival:  { icon: ICONS.ship,  color: '#2563EB' },
  trip_scheduled:  { icon: ICONS.truck, color: '#7C3AED' },
  trip_completed:  { icon: ICONS.checkSquare, color: '#16A34A' },
}
...
const NAV = [
  ...
  { to: '/planner/settings', label: 'Settings',  icon: ICONS.settings },
] as const
...
export default function PlannerLayout() {
  ...
  const plannerName = user?.name ?? null
  ...
  const PAGE_SUBTITLE: Record<string, string> = {
    ...
    '/planner/reports':  'Generate analytical reports and statistics',
  }
  ...
  const initials = plannerName ? plannerName.split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase() : '?'
  const fullName = plannerName ?? 'Planner'
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: "'Red Hat Display', ui-sans-serif, system-ui, sans-serif" }}>
      ...
      {isMobile && mobileOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'rgba(250,250,249,0.94)', backdropFilter: 'blur(28px) saturate(140%)', WebkitBackdropFilter: 'blur(28px) saturate(140%)', display: 'flex', flexDirection: 'column', alignItems: 'center', overflowY: 'auto', animation: 'menuFade 0.2s ease' }}>
          ...
          <nav style={{ width: '100%', maxWidth: 360, padding: '0 22px', display: 'flex', flexDirection: 'column', gap: 4 }}>
            {NAV.map(item => {
              const isActive = item.to === '/planner' ? pathname === '/planner' : pathname.startsWith(item.to)
              return (
                <NavLink key={item.to} to={item.to} end={item.to === '/planner'} onClick={() => setMobileOpen(false)} className="mobile-menu-item"
      ...
      <aside className={`sidebar-col${(open || isMobile) ? ' is-open' : ''}`} onClick={() => { if (isMobile) setMobileOpen(false) }}>
        <Link to="/planner" style={{ display: 'flex', alignItems: 'center', width: 40, justifyContent: 'center', transition: 'width 0.28s cubic-bezier(0.16,1,0.3,1)', ...(open ? { width: '100%' } : {}) }}>
        ...
        <nav className="nav-pill">
          {NAV.map(item => {
            const isActive = item.to === '/planner' ? pathname === '/planner' : pathname.startsWith(item.to)
            return (
              <NavLink key={item.to} to={item.to} end={item.to === '/planner'} className={() => `nav-item${isActive ? ' active' : ''}`} data-label={item.label}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, background: '#f9f9f9' }}>
        <header style={{ height: 'var(--dash-header-h)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 var(--dash-main-pad-x)', background: '#f9f9f9', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <NotificationBell categoryIcon={PLANNER_NOTIF_ICON} />
