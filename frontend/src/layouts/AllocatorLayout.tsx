...
const ALLOCATOR_NOTIF_ICON: Record<string, { icon: string; color: string }> = {
  new_trip:            { icon: ICONS.container, color: '#7C3AED' },
  maintenance_reminder: { icon: ICONS.wrench,    color: '#EA580C' },
  resource_conflict:    { icon: ICONS.truck,     color: '#DC2626' },
}

const NAV = [
  { to: '/allocator',             label: 'Dashboard',    icon: ICONS.home },
  { to: '/allocator/resources',   label: 'Resources',    icon: ICONS.truck },
  { to: '/allocator/trips',       label: 'Trips',        icon: ICONS.container },
  { to: '/allocator/maintenance', label: 'Maintenance',  icon: ICONS.wrench },
  { to: '/allocator/settings',    label: 'Settings',     icon: ICONS.settings },
] as const
...
export default function AllocatorLayout() {
  ...
  const allocatorName = user?.name ?? null
  ...
  const activeNav = NAV.find(n => pathname === n.to || (n.to !== '/allocator' && pathname.startsWith(n.to)))
  // FRD gives each Allocator screen its own literal heading (FRD 2.4.3 – 2.4.3.4).
  const PAGE_TITLE_OVERRIDE: Record<string, string> = {
    '/allocator':             'Resource Allocator Dashboard',
    '/allocator/resources':   'Resources Management',
    '/allocator/trips':       'Trip Allocation',
    '/allocator/maintenance': 'Maintenance Management',
    '/allocator/settings':    'Allocator Settings',
  }
  const title = PAGE_TITLE_OVERRIDE[pathname] ?? activeNav?.label ?? 'Dashboard'
  const PAGE_SUBTITLE: Record<string, string> = {
    '/allocator':             'Allocate resources, assign trips and manage maintenance',
    '/allocator/resources':   'Manage trucks, trailers and drivers',
    '/allocator/trips':       'Assign resources to scheduled trips',
    '/allocator/maintenance': 'Manage resource maintenance and servicing',
    '/allocator/settings':    'Configure your preferences and system settings',
  }
  ...
  const initials = allocatorName ? allocatorName.split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase() : '?'
  const fullName = allocatorName ?? 'Allocator'
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: "'Red Hat Display', ui-sans-serif, system-ui, sans-serif" }}>
      ...
      {isMobile && mobileOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'rgba(250,250,249,0.94)', backdropFilter: 'blur(28px) saturate(140%)', WebkitBackdropFilter: 'blur(28px) saturate(140%)', display: 'flex', flexDirection: 'column', alignItems: 'center', overflowY: 'auto', animation: 'menuFade 0.2s ease' }}>
          ...
          <nav style={{ width: '100%', maxWidth: 360, padding: '0 22px', display: 'flex', flexDirection: 'column', gap: 4 }}>
            {NAV.map(item => {
              const isActive = item.to === '/allocator' ? pathname === '/allocator' : pathname.startsWith(item.to)
              return (
                <NavLink key={item.to} to={item.to} end={item.to === '/allocator'} onClick={() => setMobileOpen(false)} className="mobile-menu-item"
      ...
      <aside className={`sidebar-col${(open || isMobile) ? ' is-open' : ''}`} onClick={() => { if (isMobile) setMobileOpen(false) }}>
        <Link to="/allocator" style={{ display: 'flex', alignItems: 'center', width: 40, justifyContent: 'center', transition: 'width 0.28s cubic-bezier(0.16,1,0.3,1)', ...(open ? { width: '100%' } : {}) }}>
        ...
        <nav className="nav-pill">
          {NAV.map(item => {
            const isActive = item.to === '/allocator' ? pathname === '/allocator' : pathname.startsWith(item.to)
            return (
              <NavLink key={item.to} to={item.to} end={item.to === '/allocator'} className={() => `nav-item${isActive ? ' active' : ''}`} data-label={item.label}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, background: '#f9f9f9' }}>
        <header style={{ height: 'var(--dash-header-h)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 var(--dash-main-pad-x)', background: '#f9f9f9', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <NotificationBell categoryIcon={ALLOCATOR_NOTIF_ICON} />
