...
import { ComplianceCapabilityContext, useLoadComplianceCapabilities } from '@/lib/useComplianceCapabilities'

const COMPLIANCE_NOTIF_ICON: Record<string, { icon: string; color: string }> = {
  new_activity:       { icon: ICONS.clipboard, color: '#2563EB' },
  inspection_overdue: { icon: ICONS.siren,      color: '#DC2626' },
  inspection_due:     { icon: ICONS.calendar,   color: '#EA580C' },
}

const NAV = [
  { to: '/compliance',            label: 'Dashboard',           icon: ICONS.home },
  { to: '/compliance/activities', label: 'My Activities',       icon: ICONS.clipboard },
  { to: '/compliance/completed',  label: 'Completed Activities', icon: ICONS.clipboardCheck },
  { to: '/compliance/inspections', label: 'Site Inspection',    icon: ICONS.shield },
] as const

export default function ComplianceLayout() {
  const { pathname } = useLocation()
  const { user, logout } = useAuth()
  const capState = useLoadComplianceCapabilities()
  ...
  const complianceName = user?.name ?? null
  ...
  const activeNav = NAV.find(n => pathname === n.to || (n.to !== '/compliance' && pathname.startsWith(n.to)))
  // FRD gives each Compliance screen its own literal heading (FRD 2.4.4 – 2.4.4.3).
  const PAGE_TITLE_OVERRIDE: Record<string, string> = {
    '/compliance':             'Compliance Dashboard',
    '/compliance/activities':  'My Activities',
    '/compliance/completed':   'Completed Activities',
    '/compliance/inspections': 'Site Inspection',
  }
  const title = PAGE_TITLE_OVERRIDE[pathname] ?? activeNav?.label ?? 'Dashboard'
  const PAGE_SUBTITLE: Record<string, string> = {
    '/compliance':             'Manage compliance activities, inspections, and regulatory requirements',
    '/compliance/activities':  'Current compliance inspections and tasks',
    '/compliance/completed':   'Historical compliance activities and reports',
    '/compliance/inspections': 'Schedule and manage facility inspections',
  }
  const subtitle = PAGE_SUBTITLE[pathname] ?? ''

  useEffect(() => { initToast() }, [])
  useEffect(() => { localStorage.setItem('glido-sidebar', open ? '1' : '0') }, [open])

  const initials = complianceName ? complianceName.split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase() : '?'
  const fullName = complianceName ?? 'Compliance'
  const tenantLine = tenant?.name ?? 'CFS'

  return (
    <ComplianceCapabilityContext.Provider value={capState}>
    <div style={{ display: 'flex', height: '100vh', fontFamily: "'Red Hat Display', ui-sans-serif, system-ui, sans-serif" }}>
      ...
      {isMobile && mobileOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'rgba(250,250,249,0.94)', backdropFilter: 'blur(28px) saturate(140%)', WebkitBackdropFilter: 'blur(28px) saturate(140%)', display: 'flex', flexDirection: 'column', alignItems: 'center', overflowY: 'auto', animation: 'menuFade 0.2s ease' }}>
          ...
          <nav style={{ width: '100%', maxWidth: 360, padding: '0 22px', display: 'flex', flexDirection: 'column', gap: 4 }}>
            {NAV.map(item => {
              const isActive = item.to === '/compliance' ? pathname === '/compliance' : pathname.startsWith(item.to)
              return (
                <NavLink key={item.to} to={item.to} end={item.to === '/compliance'} onClick={() => setMobileOpen(false)} className="mobile-menu-item"
      ...
      <aside className={`sidebar-col${(open || isMobile) ? ' is-open' : ''}`} onClick={() => { if (isMobile) setMobileOpen(false) }}>
        <Link to="/compliance" style={{ display: 'flex', alignItems: 'center', width: 40, justifyContent: 'center', transition: 'width 0.28s cubic-bezier(0.16,1,0.3,1)', ...(open ? { width: '100%' } : {}) }}>
        ...
        <nav className="nav-pill">
          {NAV.map(item => {
            const isActive = item.to === '/compliance' ? pathname === '/compliance' : pathname.startsWith(item.to)
            return (
              <NavLink key={item.to} to={item.to} end={item.to === '/compliance'} className={() => `nav-item${isActive ? ' active' : ''}`} data-label={item.label}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, background: '#f9f9f9' }}>
        <header style={{ height: 'var(--dash-header-h)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 var(--dash-main-pad-x)', background: '#f9f9f9', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <NotificationBell categoryIcon={COMPLIANCE_NOTIF_ICON} />
    ...
    </ComplianceCapabilityContext.Provider>
  )
}
