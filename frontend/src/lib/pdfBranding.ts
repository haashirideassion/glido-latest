...
export async function glidoLogoPng(): Promise<string | null> {
  try {
    const svgUrl = 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(GLIDO_SVG)
    const dataUrl: string = await new Promise((resolve, reject) => {
      const img = new Image()
      img.crossOrigin = 'anonymous'
      img.onload = () => {
        const canvas = document.createElement('canvas')
        canvas.width = 320; canvas.height = 62
        document.body.appendChild(canvas)
        const ctx = canvas.getContext('2d')
        if (!ctx) { document.body.removeChild(canvas); return reject(new Error('no ctx')) }
        ctx.drawImage(img, 0, 0, 320, 62)
