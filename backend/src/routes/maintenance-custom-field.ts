...
// GET is public — maintenance cards/forms need the configured label without requiring an allocator session.
router.get('/', async (req: Request, res: Response) => {
  try {
    const result = await pool.query(`SELECT field_label, field_type FROM maintenance_custom_field_settings WHERE tenant_id = $1`, [DEFAULT_TENANT_ID])
    return res.json({ success: true, data: { fieldLabel: result.rows[0]?.field_label ?? null, fieldType: result.rows[0]?.field_type ?? 'text' } })
  } catch (err) {
    console.error('[maintenance-custom-field GET /]', err)
...
router.put('/', requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    const result = await pool.query(
      `INSERT INTO maintenance_custom_field_settings (tenant_id, field_label, field_type, updated_at) VALUES ($1, $2, $3, NOW())
    ...
    console.error('[maintenance-custom-field PUT /]', err)
    return res.status(500).json({ success: false, error: { message: 'Server error' } })
  }
})
