...
export async function autoAllocatePendingTrips(tenantId: string): Promise<number> {
  try {
    ...
    for (const trip of pendingTrips.rows) {
      ...
      const truckRes = await pool.query(`SELECT id, resource_code FROM trucks WHERE tenant_id = $1 AND status = 'available' ORDER BY resource_code LIMIT 1`, [tenantId])
      ...
      await pool.query(
        `UPDATE trips SET truck_id = $1, trailer_id = $2, driver_id = $3, vehicle = $4, driver = $5,
         ...
         WHERE id = $6`,
        [truckRes.rows[0].id, trailerRes.rows[0]?.id ?? null, driverRes.rows[0].id, truckRes.rows[0].resource_code, driverRes.rows[0].driver_name, trip.id]
      )
